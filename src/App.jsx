...
// ngrok-skip-browser-warning header — added to every backend fetch
const BACKEND_HEADERS = {
  "Content-Type": "application/json",
  "ngrok-skip-browser-warning": "true",
};
const BACKEND_GET_HEADERS = {
  "ngrok-skip-browser-warning": "true",
};
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: #080c0e; color: #e2e8f0; font-family: 'DM Sans', sans-serif; min-height: 100vh; }
  .app { min-height: 100vh; display: flex; flex-direction: column; }
  .header { border-bottom: 1px solid #1a2530; padding: 0 2rem; height: 58px; display: flex; align-items: center; justify-content: space-between; background: #080c0e; position: sticky; top: 0; z-index: 10; }
  .logo { display: flex; align-items: center; gap: 10px; }
  .logo-icon { width: 28px; height: 28px; background: linear-gradient(135deg, #00d4aa, #00a3cc); border-radius: 7px; display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 600; color: #080c0e; font-family: 'JetBrains Mono', monospace; }
  .logo-text { font-size: 15px; font-weight: 600; color: #f0f4f8; letter-spacing: -0.3px; }
  .logo-sub { font-size: 11px; color: #4a6275; font-weight: 400; margin-left: 2px; }
  .status-dot { display: flex; align-items: center; gap: 6px; font-size: 11px; color: #4a6275; font-family: 'JetBrains Mono', monospace; }
  .dot { width: 6px; height: 6px; border-radius: 50%; }
  .dot.online  { background: #00d4aa; box-shadow: 0 0 6px #00d4aa88; }
  .dot.offline { background: #e05252; }
  .main { display: grid; grid-template-columns: 220px 1fr; flex: 1; min-height: calc(100vh - 58px); }
  .sidebar { border-right: 1px solid #1a2530; padding: 1.5rem 0; display: flex; flex-direction: column; gap: 2px; background: #060a0c; }
  .sidebar-label { font-size: 10px; font-weight: 600; color: #2d4255; letter-spacing: 1.5px; padding: 0 1.25rem; margin-bottom: 6px; margin-top: 4px; }
  .nav-item { display: flex; align-items: center; gap: 10px; padding: 9px 1.25rem; cursor: pointer; border: none; background: transparent; color: #4a6275; font-family: 'DM Sans', sans-serif; font-size: 13.5px; font-weight: 400; width: 100%; text-align: left; transition: all 0.15s; border-left: 2px solid transparent; }
  .nav-item:hover { color: #94b4c8; background: #0d1820; }
  .nav-item.active { color: #00d4aa; background: #0a1a16; border-left-color: #00d4aa; font-weight: 500; }
  .nav-icon { font-size: 14px; width: 18px; text-align: center; flex-shrink: 0; }
  .sidebar-divider { height: 1px; background: #1a2530; margin: 10px 1.25rem; }
  .speed-section { padding: 0 1.25rem; margin-top: 8px; }
  .speed-label-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
  .speed-label { font-size: 10px; font-weight: 600; color: #2d4255; letter-spacing: 1.5px; }
  .speed-val { font-size: 11px; color: #00d4aa; font-family: 'JetBrains Mono', monospace; }
  input[type=range].speed-slider { width: 100%; -webkit-appearance: none; height: 3px; background: #1a2a35; border-radius: 2px; outline: none; margin-bottom: 6px; }
  input[type=range].speed-slider::-webkit-slider-thumb { -webkit-appearance: none; width: 14px; height: 14px; border-radius: 50%; background: #00d4aa; cursor: pointer; box-shadow: 0 0 8px #00d4aa66; }
  .speed-marks { display: flex; justify-content: space-between; }
  .speed-mark { font-size: 9px; color: #2d4255; }
  .delay-section { padding: 0 1.25rem; margin-top: 14px; }
  .content { padding: 2rem 2.5rem; display: flex; flex-direction: column; gap: 1.25rem; overflow-y: auto; }
  .panel { background: #0d1620; border: 1px solid #1a2d3d; border-radius: 12px; padding: 1.25rem 1.5rem; animation: fadeUp 0.2s ease; }
  @keyframes fadeUp { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: translateY(0); } }
  .panel-title { font-size: 11px; font-weight: 600; color: #2d4a60; letter-spacing: 1.4px; margin-bottom: 12px; }
  .text-area { width: 100%; background: #060e14; border: 1px solid #1a2d3d; border-radius: 8px; padding: 14px 16px; color: #c8dde8; font-family: 'JetBrains Mono', monospace; font-size: 13px; line-height: 1.7; resize: vertical; outline: none; transition: border-color 0.2s; min-height: 200px; }
  .text-area:focus { border-color: #00d4aa44; }
  .text-area::placeholder { color: #1e3040; }
  .prompt-input { width: 100%; background: #060e14; border: 1px solid #1a2d3d; border-radius: 8px; padding: 12px 16px; color: #c8dde8; font-family: 'DM Sans', sans-serif; font-size: 14px; outline: none; transition: border-color 0.2s; }
  .prompt-input:focus { border-color: #00d4aa44; }
  .prompt-input::placeholder { color: #1e3040; }
  .chips { display: flex; flex-wrap: wrap; gap: 6px; margin-top: 10px; }
  .chip { font-size: 11px; padding: 4px 10px; border-radius: 20px; border: 1px solid #1e3545; background: transparent; color: #4a6275; cursor: pointer; font-family: 'DM Sans', sans-serif; transition: all 0.15s; }
  .chip:hover { border-color: #00d4aa55; color: #7ab8c8; }
  .chip.active { border-color: #00d4aa; background: #00d4aa15; color: #00d4aa; font-weight: 500; }
  .sample-chip { font-size: 11px; padding: 5px 11px; border-radius: 20px; border: 1px dashed #1e3545; background: transparent; color: #3d5a6a; cursor: pointer; font-family: 'DM Sans', sans-serif; transition: all 0.15s; text-align: left; }
  .sample-chip:hover { border-color: #00d4aa44; color: #5a8898; }
  .model-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 10px; }
  .model-card { background: #060e14; border: 1px solid #1a2d3d; border-radius: 8px; padding: 10px 12px; cursor: pointer; transition: all 0.15s; display: flex; justify-content: space-between; align-items: center; }
  ...
  .model-badge { font-size: 9px; padding: 2px 6px; border-radius: 10px; background: #1a2d3d; color: #4a6275; font-weight: 500; }
  .model-card.active .model-badge { background: #00d4aa22; color: #00d4aa; }
  .btn-primary { display: inline-flex; align-items: center; gap: 8px; padding: 11px 22px; background: #00d4aa; color: #080c0e; border: none; border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 13.5px; font-weight: 600; cursor: pointer; transition: all 0.15s; }
  .btn-primary:hover { background: #00e8bb; }
  .btn-primary:active { transform: scale(0.98); }
  .btn-primary:disabled { background: #1a2d3d; color: #2d4a60; cursor: not-allowed; }
  .btn-danger { display: inline-flex; align-items: center; gap: 8px; padding: 11px 22px; background: #e05252; color: #fff; border: none; border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 13.5px; font-weight: 600; cursor: pointer; animation: pulse 1.5s ease-in-out infinite; }
  .btn-warn { display: inline-flex; align-items: center; gap: 8px; padding: 11px 22px; background: #d48800; color: #fff; border: none; border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 13.5px; font-weight: 600; cursor: pointer; }
  .btn-ghost { display: inline-flex; align-items: center; gap: 6px; padding: 10px 16px; background: transparent; color: #4a6275; border: 1px solid #1a2d3d; border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 13px; cursor: pointer; transition: all 0.15s; }
  .btn-ghost:hover { border-color: #2a4a60; color: #7ab8c8; }
  .btn-ghost:disabled { opacity: 0.3; cursor: not-allowed; }
  .btn-ai { display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; background: #0d1a28; color: #00d4aa; border: 1px solid #00d4aa44; border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 500; cursor: pointer; transition: all 0.15s; margin-top: 12px; }
  ...
  .char-count { font-size: 11px; color: #2d4a60; font-family: 'JetBrains Mono', monospace; margin-left: auto; }
  .status-bar { display: flex; align-items: center; gap: 10px; padding: 10px 14px; background: #060e14; border: 1px solid #1a2d3d; border-radius: 8px; font-size: 12.5px; animation: fadeUp 0.2s ease; }
  ...
  .voice-btn { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10px; width: 100%; padding: 2.5rem; background: #060e14; border: 1px dashed #1a2d3d; border-radius: 10px; cursor: pointer; transition: all 0.2s; font-family: 'DM Sans', sans-serif; }
  .voice-btn:hover { border-color: #e0525244; background: #120808; }
  .voice-btn.listening { border-color: #e05252; background: #120808; animation: borderPulse 1.5s ease-in-out infinite; }
  @keyframes borderPulse { 0%,100% { border-color: #e05252; } 50% { border-color: #e0525266; } }
  .mic-ring { width: 64px; height: 64px; border-radius: 50%; background: #1a0808; border: 2px solid #e05252; display: flex; align-items: center; justify-content: center; font-size: 26px; transition: all 0.2s; }
  .voice-btn.listening .mic-ring { background: #e05252; box-shadow: 0 0 20px #e0525255; }
  .voice-label { font-size: 13px; color: #4a6275; font-weight: 500; }
  .voice-btn.listening .voice-label { color: #e07070; }
  .template-row { display: flex; align-items: center; gap: 10px; padding: 12px 14px; background: #060e14; border: 1px solid #1a2d3d; border-radius: 8px; transition: border-color 0.15s; }
  .template-row:hover { border-color: #2a4050; }
  .tpl-name { font-size: 13px; font-weight: 500; color: #7ab8c8; flex: 1; }
  .tpl-hotkey { font-size: 10px; padding: 2px 7px; border-radius: 4px; background: #1a2d3d; color: #4a6275; font-family: 'JetBrains Mono', monospace; }
  .tpl-preview { font-size: 11px; color: #2d4a60; margin-top: 2px; }
  .tpl-actions { display: flex; gap: 6px; }
  .tpl-btn { font-size: 11px; padding: 3px 9px; border-radius: 5px; border: 1px solid #1a2d3d; background: transparent; cursor: pointer; font-family: 'DM Sans', sans-serif; color: #4a6275; transition: all 0.15s; }
  ...
  .save-row input { background: #060e14; border: 1px solid #1a2d3d; border-radius: 7px; padding: 9px 12px; color: #c8dde8; font-family: 'DM Sans', sans-serif; font-size: 13px; outline: none; transition: border-color 0.2s; }
  .save-row input:focus { border-color: #00d4aa44; }
  .save-row input::placeholder { color: #1e3040; }
  .key-section { background: #060e14; border: 1px solid #1a2d3d; border-radius: 8px; padding: 12px 14px; margin-bottom: 4px; }
  .key-label { font-size: 10px; font-weight: 600; color: #2d4a60; letter-spacing: 1.3px; margin-bottom: 8px; }
  .key-input { width: 100%; background: #080c0e; border: 1px solid #1a2d3d; border-radius: 6px; padding: 9px 12px; color: #c8dde8; font-family: 'JetBrains Mono', monospace; font-size: 12px; outline: none; transition: border-color 0.2s; }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
      try {
        const r = await fetch(`${API_BASE}/status`, { headers: BACKEND_GET_HEADERS });
  ...
  const fetchTemplates = async () => {
    try {
      const r = await fetch(`${API_BASE}/templates`, { headers: BACKEND_GET_HEADERS });
      const d = await r.json();
  ...
  const handleStop = async () => {
    ...
    try { await fetch(`${API_BASE}/stop`, { method: "POST", headers: BACKEND_HEADERS }); } catch {}
  ...
  const deleteTemplate = async (name) => {
    try {
      await fetch(`${API_BASE}/templates/${encodeURIComponent(name)}`, {
        method: "DELETE", headers: BACKEND_GET_HEADERS,
      });
      fetchTemplates();
    } catch {}
  };
  ...
  const modelShort = serverModel
    ? serverModel.replace("llama-","").replace("-versatile","").replace("-instant","")
    : "";
        ...
        <div className="main">
          <aside className="sidebar">
            ...
            <div className="speed-section">
              ...
              <div className="speed-marks">{SPEED_OPTIONS.map(s => <span key={s.id} className="speed-mark">{s.label}</span>)}</div>
            </div>
            <div className="delay-section">
              ...
              <p style={{ fontSize: 10, color: "#1e3040", marginTop: 6, lineHeight: 1.5 }}>Seconds to switch window before typing starts.</p>
            </div>
          </aside>
          ...
          <main className="content">
            {mode === "ai" && (
              <div className="panel">
                ...
                <input className="prompt-input" value={aiPrompt} onChange={e => setAiPrompt(e.target.value)}
                  placeholder="e.g. Write an email to HR requesting sick leave…"
            ...
            {mode === "voice" && (
              <div className="panel">
                ...
                <div className={`voice-btn ${isListening ? "listening" : ""}`}
                  onClick={isListening ? () => recognitionRef.current?.stop() : handleVoice}>
                  <div className="mic-ring">🎙</div>
                  <span className="voice-label">{isListening ? "Listening — click to stop" : "Click to start recording"}</span>
            ...
            {mode === "templates" && (
              <div className="panel">
                ...
                {templates.length === 0
                  ? <p style={{ fontSize: 13, color: "#2d4a60" }}>No templates yet. Load text and save below.</p>
                ...
                <div className="panel-title" style={{ marginTop: 8 }}>SAVE CURRENT TEXT</div>
              ...
              <div className="action-bar">
                ...
                {!backendOnline && (
                  <span style={{ fontSize: 11, color: "#e07070", marginLeft: "auto" }}>
                    Backend offline — run python main.py + ngrok
                  </span>
                )}
              </div>
              {status.msg && (
